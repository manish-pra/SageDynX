...
class Drone(object):
    ...
    def initialize_plot_handles(self, fig_gp, fig_dyn=None):
        ...
        if plot_optimal_path:
            ...
            traj_iter = 0
            opt_a_file = open(save_path + str(traj_iter) + "/data_lap1.pkl", "rb")
            ...
            ax.plot(opt_physical_state_traj[:,0], opt_physical_state_traj[:,1], color="tab:green", linewidth=1.6, label="Optimal")
            ...
            ax.set_xlim(-6.0, 6.0)
            ax.grid(False)
            fig_gp.tight_layout(pad=0.0)
            ax.plot(-10,10, "--", color="black", label="SAGE-DynX")
            ax.legend(fontsize='small', labelspacing=0.2, handlelength=2, loc='lower left')
